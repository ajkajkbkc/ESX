...
static status_t flexspi_nor_wait_bus_busy_internal(FLEXSPI_Type *base)
{
    /* Wait status ready. */
    bool isBusy;
    uint32_t readValue;
...
static status_t flexspi_nor_write_enable_internal(FLEXSPI_Type *base, uint32_t baseAddr)
{
    flexspi_transfer_t flashXfer;
    status_t status;
...
static status_t flexspi_nor_flash_erase_sector_internal(FLEXSPI_Type *base, uint32_t address)
{
    status_t status;
    flexspi_transfer_t flashXfer;
    ...
    status = flexspi_nor_wait_bus_busy_internal(base);

    return status;
}

static status_t flexspi_nor_enable_quad_mode_internal(FLEXSPI_Type *base)
{
    flexspi_transfer_t flashXfer;
    status_t status;
    ...
    /* Write neable */
    status = flexspi_nor_write_enable_internal(base, 0);
...
static int32_t mflash_drv_erase_sector_internal(uint32_t sector_addr)
{
    ...
    status_t status;
    status = flexspi_nor_flash_erase_sector_internal(MFLASH_FLEXSPI, sector_addr);
    if (status != kStatus_Success)
    {
        retVal = -1;
    }
...
static status_t flexspi_nor_flash_page_program_internal(FLEXSPI_Type *base, uint32_t address, const uint32_t *src)
{
    status_t status;
    flexspi_transfer_t flashXfer;

    /* Write enable */
    status = flexspi_nor_write_enable_internal(base, address);
...
static status_t flexspi_nor_flash_page_program_internal_bytes(FLEXSPI_Type *base, uint32_t address, const uint32_t *src, size_t size)
{
    status_t status;
    flexspi_transfer_t flashXfer;
...
static int32_t mflash_drv_write_page_internal(uint32_t page_addr, uint32_t *data, size_t size)
{
    __asm("cpsid i");
    ...
    if (size == 0)
    {
        status = flexspi_nor_flash_page_program_internal(MFLASH_FLEXSPI, page_addr, data);
    }
    else
    {
        status = flexspi_nor_flash_page_program_internal_bytes(MFLASH_FLEXSPI, page_addr, data, size);
    }
...
int32_t mflash_drv_write_page(void *page_addr, uint32_t *data)
{
    ...
    volatile int32_t result;
    result = mflash_drv_write_page_internal((uint32_t)page_addr, data, 0);
    return result;
}
int32_t mflash_drv_write_page_bytes(void *page_addr, uint32_t *data, size_t size)
{
    volatile int32_t result;
    result = mflash_drv_write_page_internal((uint32_t)page_addr, data, size);
    return result;
}
...
static int32_t mflash_drv_init_internal(void)
{
    ...
    /* Enter quad mode. */
    status = flexspi_nor_enable_quad_mode_internal(MFLASH_FLEXSPI);
    if (status != kStatus_Success)
    {
        return -1;
    }
