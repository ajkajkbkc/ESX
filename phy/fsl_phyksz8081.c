...
#if (KALYKE_PHY_TIMEOUT_COUNT == 1)
#define PHY_TIMEOUT_COUNT 50000
#else
#define PHY_TIMEOUT_COUNT 80000
#endif
...
status_t PHY_Kalyke_Get_Something(phy_handle_t *handle, uint32_t *pBASICSTATUS_REG)
{
    status_t result = kStatus_Success;
    uint32_t data = 0;
    ...
    if (result == kStatus_Success)
    {
        *pBASICSTATUS_REG = data;
    }
    return result;
}

status_t PHY_Kalyke_Get_ID1(phy_handle_t *handle, uint32_t *pID1)
{
    status_t result = kStatus_Success;
    uint32_t idReg = 0;
...
status_t PHY_Kalyke_Get_ID2(phy_handle_t *handle, uint32_t *pID2)
{
    status_t result = kStatus_Success;
    uint32_t idReg = 0;

    /* Read the basic status register. */
    result = MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_ID2_REG, &idReg);
    if (result == kStatus_Success)
    {
        *pID2 = idReg;
    }
...
void PHY_Kalyke_Read_Others(phy_handle_t *handle, uint32_t *pBuf)
{
    uint32_t i = 0;
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_BASICCONTROL_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_AUTONEG_ADVERTISE_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_AUTONEG_LINKPARTNER_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_AUTONEG_EXPANSION_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_1000BASET_CONTROL_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_MMD_ACCESS_CONTROL_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_MMD_ACCESS_DATA_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_CONTROL1_REG, pBuf + i++);
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_CONTROL2_REG, pBuf + i++); //9
}

void PHY_Kalyke_Read_Crash(phy_handle_t *handle, uint32_t *pBuf)
{
    uint32_t i = 0;
    MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_BASICCONTROL_REG, pBuf + i++);
#if 0
