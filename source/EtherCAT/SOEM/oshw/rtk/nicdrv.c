...
uint16 secMAC[3] = { 0x0404, 0x0404, 0x0404 };
...
#if 0
void mtx_lock(SemaphoreHandle_t mu)
{
    while( xSemaphoreTake( mu, portMAX_DELAY ) != pdPASS );
}

void mtx_unlock(SemaphoreHandle_t mu)
{
    xSemaphoreGive(mu);
}
#else
...
{
   LOGD(TAG, "Enter %s(), port = 0x%08X, ifname = %s, secondary = %d", __func__, port, ifname, secondary);
   int i;
   int rVal;
   int *psock;
   ...
   LOGV(TAG, "%s : 00000000000", __func__);
   get_Mac((uint8_t *)priMAC, 1);
   LOGV(TAG, "%s : 00000000001", __func__);
   get_Mac((uint8_t *)secMAC, 0);
   LOGV(TAG, "%s : 00000000002", __func__);
