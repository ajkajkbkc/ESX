               ...
               boolean CA, int *psize, void *p, int timeout)
{
   ...
   uint8 cnt, toggle;
   boolean NotLast;
   EC_PRINT("Enter %s(), slave = %u, index = 0x%04X, subindex = %u", __func__, slave, index, subindex);
   ec_clearmbx(&MbxIn);
   /* Empty slave out mailbox if something is in. Timeout set to 0 */
   wkc = ecx_mbxreceive(context, slave, (ec_mbxbuft *)&MbxIn, 0);
   EC_PRINT("ecx_SDOread: ecx_mbxreceive return wkc = %d", wkc);
   ec_clearmbx(&MbxOut);
   aSDOp = (ec_SDOt *)&MbxIn;
   SDOp = (ec_SDOt *)&MbxOut;
   ...
   EC_PRINT("ecx_SDOread: cnt = %u", cnt);
   ...
   EC_PRINT("ecx_SDOread: send CoE request to slave, SDOp->Index = 0x%04X, SDOp->SubIndex = %u", SDOp->Index, SDOp->SubIndex);
   hexdump(MbxOut, sizeof(*SDOp));
   /* send CoE request to slave */
   wkc = ecx_mbxsend(context, slave, (ec_mbxbuft *)&MbxOut, EC_TIMEOUTTXM);
   EC_PRINT("ecx_SDOread: wkc = %d", wkc);
               ...
               boolean CA, int *psize, void *p, int timeout)
{
   ...
   //EC_PRINT("&Command = 0x%p, &Index = 0x%08X, &SubIndex = 0x%08X", &SDOp->Command, &SDOp->Index, &SDOp->SubIndex);
   SDOp->Index = htoes(index);
   if (CA && (subindex > 1))
   {
      subindex = 1;
...
//   hexdump(MbxOut, sizeof(*SDOp));
   ...
   if (wkc > 0) /* succeeded to place mailbox in slave ? */
   {
      ...
      /* read slave response */
      wkc = ecx_mbxreceive(context, slave, (ec_mbxbuft *)&MbxIn, timeout);
      EC_PRINT("ecx_SDOread: wkc = %d......002", wkc);
      if (wkc > 0) /* succeeded to read slave response ? */
      {
         /* slave response should be CoE, SDO response and the correct index */
         if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) &&
...
int ecx_readPDOmap(ecx_contextt *context, uint16 Slave, int *Osize, int *Isize)
{
   ...
   *Osize = 0;
   ...
   EC_PRINT("ecx_readPDOmap: wkc = %d, nSM = %u", wkc, nSM);
   /* positive result from slave ? */
   if ((wkc > 0) && (nSM > 2))
   {
...
{
   EC_PRINT("Enter %s(), Slave = %u, Thread_n = %d", __func__, Slave, Thread_n);
   int wkc, rdl;
   int retVal = 0;
   uint8 nSM, iSM, tSM;
   int Tsize;
