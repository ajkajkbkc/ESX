...
int ecx_mbxreceive(ecx_contextt *context, uint16 slave, ec_mbxbuft *mbx, int timeout)
{
   ...
   EC_PRINT("configadr = 0x%04X, mbxl = %u", configadr, mbxl);
   if ((mbxl > 0) && (mbxl <= EC_MAXMBX))
   {
      osal_timert timer;
      ...
      do /* wait for read mailbox available */
      {
         ...
         EC_PRINT("ecx_FPRD return wkc = %d, SMstat = 0x%X", wkc, SMstat);
         SMstat = etohs(SMstat);
         if (((SMstat & 0x08) == 0) && (timeout > EC_LOCALDELAY))
         {
            osal_usleep(EC_LOCALDELAY);
 ...
 */
RAMFUNCTION_SECTION_CODE(static void ecx_pushindex(ecx_contextt *context, uint8 idx, void *data, uint16 length, uint16 DCO))
{
 ...
 */
RAMFUNCTION_SECTION_CODE(static int ecx_pullindex(ecx_contextt *context))
{
   int rval = -1;
 ...
 */
RAMFUNCTION_SECTION_CODE(int ecx_main_send_processdata(ecx_contextt *context, uint8 group, boolean use_overlap_io))
{
   uint32 LogAdr;
   uint16 w1, w2;
 ...
 * @return Work counter.
 */
RAMFUNCTION_SECTION_CODE(int ecx_receive_processdata_group(ecx_contextt *context, uint8 group, int timeout))
{
   int pos, idx;
   int wkc = 0, wkc2;
   uint16 le_wkc = 0;
