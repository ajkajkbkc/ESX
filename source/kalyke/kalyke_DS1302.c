...
// GPIO Pins
#define DS1302_GPIO	GPIO3
#define DS1302_SCLK	(0x00001000U)
#define DS1302_SDA	(0x00002000U)
#define DS1302_RST	(0x00000010U)


#define DS1302_SDA_NUM  13
#define DS1302_SCLK_NUM 12
#define DS1302_RST_NUM  4
...
#endif
    gpio_pin_config_t configOutput =
    {
        kGPIO_DigitalOutput, 0,
        kGPIO_NoIntmode,
    };
    GPIO_PinInit(DS1302_GPIO, DS1302_SDA_NUM, &configOutput);
}
...
static void readSDA(void)
{
    ...
    HAL_GPIO_Init(DS1302_GPIO, &GPIO_InitStructure);
#endif
    gpio_pin_config_t configInput =
    {
        kGPIO_DigitalInput, 0,
        kGPIO_NoIntmode,
    };
    GPIO_PinInit(DS1302_GPIO, DS1302_SDA_NUM, &configInput);
}

#define GPIO_PIN_RESET    0U
#define GPIO_PIN_SET      1U
static inline void HAL_GPIO_WritePin(GPIO_Type *base, uint32_t mask, uint32_t PinState)
{
...
static void DS1302_SendCmd(uint8_t cmd)
{
    ...
    for (i = 0; i < 8; i ++)
    {
        ...
        if (cmd & 1)
        {
            GPIO_PortSet(DS1302_GPIO, DS1302_SDA);
        }
        else
        {
            GPIO_PortClear(DS1302_GPIO, DS1302_SDA);
        }
        
        //		DS1302_SCK = 1;
        //HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SCLK,  GPIO_PIN_SET);
        GPIO_PortSet(DS1302_GPIO, DS1302_SCLK);
        delayUS_DWT(1);

        //		DS1302_SCK = 0;
        //HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SCLK,  GPIO_PIN_RESET);
        GPIO_PortClear(DS1302_GPIO, DS1302_SCLK);
        delayUS_DWT(1);
        cmd >>= 1;
    }
}
...
static uint8_t DS1302_ReadByte(uint8_t addr)
{
    ...
    for (i = 0; i < 8; i ++)
    {
        ...
        if (GPIO_PinReadPadStatus(DS1302_GPIO, DS1302_SDA_NUM))
        {
            temp |= 0x80;
        }
        //		DS1302_SCK = 1;
...
void DS1302_ReadTime2(uint8_t *buf)
{
    uint8_t tmp;

    buf[1] = DS1302_ReadByte(DS1302_YEAR);

    buf[2] = DS1302_ReadByte(DS1302_MONTH);

    buf[3] = DS1302_ReadByte(DS1302_DATE);

    buf[4] = DS1302_ReadByte(DS1302_HOUR);

    buf[5] = DS1302_ReadByte(DS1302_MIN);

    buf[6] = DS1302_ReadByte((DS1302_SEC)) & 0x7F;

    buf[7] = DS1302_ReadByte(DS1302_DAY);
}
...
void DS1302_Init(void)
{
    ...
    GPIO_PinInit(DS1302_GPIO, DS1302_SCLK_NUM, &config);
    GPIO_PinInit(DS1302_GPIO, DS1302_SDA_NUM, &config);
    GPIO_PinInit(DS1302_GPIO, DS1302_RST_NUM, &config);
